...
func main() {
	...
	specParaCron := flag.String("spec", "", "Especificación para cron")
	flag.Parse()
	if *archivoRespaldar == "" || *tokenTelegram == "" || *idChat == "" || *specParaCron == "" {
		flag.PrintDefaults()
		return
	}
	miCron := cron.New()
	errorIniciandoCron := miCron.AddFunc(*specParaCron, func() {
	...
	if errorIniciandoCron != nil {
		fmt.Printf("Error iniciando cron: %v", errorIniciandoCron)
		return
	}
	miCron.Start()
	select {}
